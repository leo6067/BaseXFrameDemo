...
import com.xy.demo.ui.ac.ACSaveRemoteActivity
...
class DeviceAdapter : BaseQuickAdapter<RemoteModel, BaseViewHolder>(R.layout.item_device) {
	override fun convert(holder: BaseViewHolder, item: RemoteModel) {
		...
		Globals.log("XXXXXXXXXXXacOrderMode item " + item.toString())
		...
		if (item.type == 1) {
			...
			typeIV.visibility = View.INVISIBLE
			tvIcon.setBackgroundResource(R.drawable.ac_item_ac)
		}
	...
	fun popupWindow(context: Context, view: View?, remoteModel: RemoteModel) {
		...
		editTV.setOnClickListener {
			...
			if (remoteModel.type ==1){
				...
				val intent = Intent()
				intent.putExtra(Constants.KEY_REMOTE, remoteModel)
				intent.setClass(context, ACSaveRemoteActivity::class.java)
				context.startActivity(intent)
			}
			popupWindow.dismiss()
		}

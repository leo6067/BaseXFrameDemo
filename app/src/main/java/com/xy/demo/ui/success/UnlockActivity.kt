...
import com.xy.demo.databinding.ActivityUnlockBinding
...
//解锁成功
class UnlockActivity : MBBaseActivity<ActivityUnlockBinding,MainViewModel>() {
	...
	companion object {
		fun newInstance(activity: Activity, filePath :String) {   //doType  1：压缩 2 加密 3 解密 4 提取文字  5 提取图片
			...
			intent.setClass(activity, UnlockActivity::class.java)
...
//			binding.compressTV -> DialogManage.showCompressDialog(this@PdfCreateActivity,)
			binding.shareTV -> 	CompressUtil.shareFile(this@UnlockActivity, filePath)
			binding.openTV -> {
				try {
					// 尝试加载 PDF 文件，不提供密码
					val document = PDDocument.load(File(filePath))
					showLoading()
					PDFActivity.setNewInstance(this@UnlockActivity, filePath, file.name, 2)
				} catch (e: InvalidPasswordException) {
					DialogManage.showDecodeDialog(this@UnlockActivity, filePath, file.name, 1)
				} catch (e: IOException) {
